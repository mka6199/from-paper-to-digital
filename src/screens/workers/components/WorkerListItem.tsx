...
function WorkerListItem(props: Props) {
      ...
      <View style={{ alignItems: 'flex-end', gap: 6, marginLeft: spacing.md }}>
        ...
        {showMoney ? (
          <Text style={[typography.small, { color: colors.text }]}>
            {format(Number(monthlySalaryAED))}
          </Text>
        ) : null}
      </View>
    </Pressable>
...
export default React.memo(WorkerListItem, (prev, next) => {
  return (
    prev.name === next.name &&
    prev.role === next.role &&
    prev.employeeId === next.employeeId &&
    prev.monthlySalaryAED === next.monthlySalaryAED &&
    prev.dueAtMs === next.dueAtMs
  );
});
